...
<head>
    <meta charset="UTF-8">
    <title><?= $modoEdicion ? 'Editar serie' : 'Añadir serie' ?></title>
...
<div class="container py-4 py-lg-5">
    <div class="admin-page-head">
        <div>
            <h1><?= $modoEdicion ? 'Editar serie' : 'Añadir serie' ?></h1>
            <p><?= $modoEdicion ? 'Modifica los datos de la serie.' : 'Crea una nueva serie.' ?></p>
        </div>
    ...
    <div class="admin-glass-card p-4">
        <form method="POST" action="<?= htmlspecialchars(mm_admin_url('series/save.php')) ?>" enctype="multipart/form-data">
            ...
            <input type="hidden" name="poster_actual" value="<?= htmlspecialchars($serie['poster'] ?? '') ?>">
            <input type="hidden" name="banner_actual" value="<?= htmlspecialchars($serie['banner'] ?? '') ?>">

            <div class="mb-3">
                <label class="form-label">Título</label>
                <input type="text" name="titulo" class="form-control" required value="<?= htmlspecialchars($serie['titulo'] ?? '') ?>">
            </div>
            ...
            <div class="mb-3">
                ...
                <?php if (!empty($serie['poster'])): ?>
                    <div class="mt-3">
                        ...
                        <img src="<?= htmlspecialchars(mm_asset_url($serie['poster'])) ?>" alt="Poster actual" style="max-width:180px;border-radius:10px;">
                    </div>
                <?php endif; ?>

                <small class="text-secondary d-block mt-2">
                    Si no subes una imagen nueva, se mantiene la actual. Se guarda en assets/img/series/posters y se convierte a WebP cuando PHP tiene GD con soporte WebP.
                </small>
            </div>
            ...
            <div class="mb-3">
                ...
                <?php if (!empty($serie['banner'])): ?>
                    <div class="mt-3">
                        ...
                        <img src="<?= htmlspecialchars(mm_asset_url($serie['banner'])) ?>" alt="Banner actual" style="max-width:280px;border-radius:10px;">
                    </div>
                <?php endif; ?>

                <small class="text-secondary d-block mt-2">
                    Si no subes una imagen nueva, se mantiene la actual. Se guarda en assets/img/series/banners y se convierte a WebP cuando PHP tiene GD con soporte WebP.
                </small>
            </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="edad" class="form-control" value="<?= htmlspecialchars($serie['edad'] ?? '') ?>" placeholder="+16, +18, TP...">
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="id_genero" class="form-select">
                        ...
                        <?php foreach ($generos as $genero): ?>
                            <option value="<?= (int)$genero['id'] ?>" <?= ((int)($serie['id_genero'] ?? 0) === (int)$genero['id']) ? 'selected' : '' ?>>
                ...
                <div class="col-md-6">
                    ...
                    <select name="id_plataforma" class="form-select">
                        ...
                        <?php foreach ($plataformas as $plataforma): ?>
                            <option value="<?= (int)$plataforma['id'] ?>" <?= ((int)($serie['id_plataforma'] ?? 0) === (int)$plataforma['id']) ? 'selected' : '' ?>>
            ...
            <div class="form-check mt-3 mb-3">
                <input class="form-check-input" type="checkbox" name="destacada" id="destacada" <?= ((int)($serie['destacada'] ?? 0) === 1) ? 'checked' : '' ?>>
